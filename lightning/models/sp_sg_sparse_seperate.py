...
class SPSGSparseSeparate(nn.Module):
    default_conf = {
            "d_model": 256,
            "nhead": 4,
            "nlayer": 4,
    ...
    def __init__(self, config):
        ...
        self.kpt_extractor  = SuperPoint(config=self.kpt_conf)
        self.encoder        = FeatureEncoder()

        self.feat_transformer = AttentionalGNN(feature_dim=self.conf['d_model'],
                                              no_layers=self.conf['nlayer'])
        self.mapping        = MLP([self.conf['d_model'], 512, 1024, 3])

        # For sparse, we need to freeze the encoder
        for param in self.kpt_extractor.parameters():
            param.requires_grad = False


    def forward(self, data:dict):
        #import pdb; pdb.set_trace()
        x           = self.kpt_extractor(data)
        keypoints   = x['keypoints']
        scores      = x['scores']

        fmap        = self.encoder(data['image'])
        B, N, H, W  = fmap.shape
        fmap        = fmap.reshape(B, N, H*W)
        #fmap        = fmap.reshape(B, H*W, N)
        fmap        = self.feat_transformer(fmap)
        fmap        = fmap.reshape(B, N, H, W)
...
if __name__ == "__main__":
    ...
    model = SPSGSparseSeparate(config={})
